        ...
        private void SelectingPerformingOperation(Operator op)
        {
            ...
            if (op.symbolOperator == 'R')
            {
                if (operands.Count == 7)
                {
                    ...
                    if (points == null)
                    {
                        MessageBox.Show($"Массива точек {name} не существует.");
                        comboBox1.Items.Add($"Массива точек {name} не существует.");
                        return;
                    }
                    points.ResetPoints(x1, x2, x3, y1, y2, y3);
                    comboBox1.Items.Add($"Массив точек {name} изменен.");
                }
                else
                {
            ...
            else if (op.symbolOperator == 'T')
            {
                if (operands.Count == 2)
                {
                    ...
                    Point[] temp_points = new Point[3];
                    points.points.CopyTo(temp_points, 0);
                    Triangle triangle = new Triangle(tri_count + 1, temp_points, name);
                    ...
                    Console.WriteLine(points.points[1]);
                    Console.WriteLine(points.points[2]);
